...
function genCardCode(cardOptions) {
    ...
    for (const key in cardOptions) {
        switch (cardOptions[key]) {
            case "cardTitle":
                const titleText = h(document.getElementById(cardOptions[key] + "Txt").value);
                title = '\t\t<h5 class="card-title">' + titleText + '</h5>\n';
                break;

            case "cardSubtitle":
                const subtitleText = h(document.getElementById(cardOptions[key] + "Txt").value);
                subtitle = '\t\t<h6 class="card-subtitle">' + subtitleText + '</h6>\n';
                break;

            case "cardText":
                const textText = h(document.getElementById(cardOptions[key] + "Txt").value).replace(/[\r\n]/g, '<br>\n\t\t\t');
